...
import auth.action.Authentication
...
@Singleton
class AuthController @Inject()(cc: ControllerComponents, userRepo: UserRepository, authentication: Authentication) extends AbstractController(cc) {
    ...
    userRepo.get(request.body.email) match {
      case Some(user) => {
        if (BCrypt.checkpw(request.body.password, user.password)) {
          val token = JWT.encode(authentication.KEY, Json.toJson(user), algorithm = Option(Algorithm.HS256))
